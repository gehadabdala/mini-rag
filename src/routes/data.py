# Routers (routers/) Handles API endpoints for specific features.
...
# request بيكون شايل ال app اللي موجود في ال main
async def upload_data(
    ...
    file_path, file_id = data_controller.generate_unique_filename(
        orig_filename=file.filename, project_id=project_id
    )

    try:
        async with aiofiles.open(file_path, "wb") as f:
            while chunk := await file.read(
                app_setting.FILE_DEFAULT_CHUNK_SIZE
            ):  # Read file in chunks
                await f.write(chunk)
...
async def process_endpoint(
    ...
    if process_request.file_id:
        asset_record = await asset_model.get_asset_record(
            asset_project_id=project.project_id, asset_name=str(process_request.file_id)
        )
    ...
    process_controller = ProcessController(
        project_id=project_id, db_client=request.app.db_client
    )
    ...
    for asset_id, file_id in project_files_ids.items():  # _id =>key
