...
class QdrantDBProvider(VectorDBInterface):
    ...
    def insert_one(
        ...
        if not self.is_collection_exists(collection_name):
            self.logger.error(
                f"Can not insert record to collection {collection_name} because it does not exist"
            )
            return False
                ...
                records=[
                    models.Record(
                        ...
                        payload={
                            "text": text,
                            "metadata": metadata,
                        },  # ممكن نحتاج نرجع نعدلها عشان نضيف ال metadata في شكل مناسب لل qdrant
                    )
                ],
            )
        except Exception as e:
            self.logger.error(
                f"Error inserting record to collection {collection_name}: {str(e)}"
            )
            return False
    ...
    def insert_many(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                self.logger.error(
                    f"Error inserting batch starting at index {i} to collection {collection_name}: {str(e)}"
                )
                return False
        return True
