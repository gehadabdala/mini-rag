...
class CohereProvider(LLMInterface):
    ...
    def generate_text(
        ...
        # ملحوظة: في مكتبة cohere الجديدة بنستخدم message مش messages
    ...
    def embed_text(self, text: str, document_type: str = None):
        ...
        input_type = CoHereEnums.DOCUMENT.value  # تأكدي من السبيلنج DOCUMENT
        ...
        if not response or not response.embeddings or not response.embeddings.float:
            self.logger.error("Error generating embeddings with Cohere")
            return None
        if isinstance(text, list):
